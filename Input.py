...
class Variables:
    ...
    def __init__(self, fname="", interactive=0):
        ...
        if fname == '' and interactive == 1:
            ...
            print('Or run with "interactive=1"'
        ...
        self.step_rotational\
            = 2*self.diffusion_constant_rotational/self.rms_velocity_rotational
        ...
        if self.run_behaviour is True:
            ...
            if self.tumble_behaviour is True:
                ...
                self.tumble_length_mean\
                    = int(np.round(self.tumble_duration_mean/self.base_time))
                self.tumble_duration_variation\
                    = self.bact.getboolean('tumble_duration_var')
                self.tumble_angle_mean\
                    = np.deg2rad(self.bact.getfloat('tumble_angle_mean'))
                self.tumble_angle_variation\
                    = self.bact.getboolean('tumble_angle_var')
            else:
                ...
                self.pause_length_mean\
                    = int(np.round(self.pause_duration_mean/self.base_time))
        self.success = 0
